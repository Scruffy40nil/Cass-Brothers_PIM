 ...
 * NEW: WEBHOOK INTEGRATION for PIM AI System
 */
...
function onOpen() {
  try {
    ...
    ui.createMenu('PIM Data Processor')
      ...
      .addSeparator()
      .addItem('🔗 Test Webhook Integration', 'testWebhookIntegration')
      .addToUi();
 ...
 * Automatically triggered when AI extraction completes
 */
function doPost(e) {
  try {
    console.log('🔄 Webhook received from PIM AI system...');
    ...
    if (data.source !== 'PIM_AI_System') {
      console.log('❌ Unauthorized webhook request - invalid source');
    ...
    if (data.trigger_cleaning && rowNumber) {
      try {
        // Clean the specific row that was processed by AI
        cleanSingleRow(rowNumber);

        const successMessage = `Row ${rowNumber} cleaned after ${operationType}`;
        ...
        console.error('❌ Error during row cleaning:', cleaningError);

        return ContentService
          .createTextOutput(JSON.stringify({
            'success': false,
            'error': `Cleaning failed: ${cleaningError.toString()}`,
    ...
    console.log('⚠️ Invalid webhook request - missing required parameters');
    return ContentService
      .createTextOutput(JSON.stringify({'success': false, 'error': 'Invalid request parameters'}))
      .setMimeType(ContentService.MimeType.JSON);
...
/**
 * Test webhook integration (for debugging)
 */
function testWebhookIntegration() {
  ...
  try {
    ...
    const result = doPost(mockWebhookData);
    ...
    if (response.success) {
      ui.alert(`✅ Webhook Test Successful!\n\nResponse: ${response.message}\nRow: ${response.row_processed}\nOperation: ${response.operation_type}\n\n🎯 Your webhook is ready to receive calls from the PIM AI system!`);
    } else {
      ui.alert(`⚠️ Webhook Test Warning:\n\nError: ${response.error}\n\nThis might be expected if row 2 is empty or has no title.`);
    }

  } catch (error) {
    console.error('❌ Webhook test failed:', error);
    ui.alert(`❌ Webhook Test Failed: ${error.toString()}`);
  }
}

// ===============================
// CHECKBOX TRIGGER SYSTEM
// ===============================
