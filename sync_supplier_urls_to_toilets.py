#!/usr/bin/env python3
"""
Sync Supplier URLs from SQLite Database to Toilets Google Sheet
Matches products by SKU and updates the url column (Column A)
"""
...
def sync_supplier_urls_to_toilets():
    """
    Read supplier URLs from supplier_products.db and write them to the Toilets sheet
    """

    print("=" * 80)
    print("SYNC SUPPLIER URLs TO TOILETS GOOGLE SHEET")
    print("=" * 80)
    ...
    # Get all toilets products from database
    cursor.execute("""
        SELECT sku, product_url, supplier_name, detected_collection
        FROM supplier_products
        WHERE detected_collection IN ('toilets', 'toilet', 'wc', 'pan')
        ORDER BY sku
    """)

    db_products = cursor.fetchall()
    print(f"   ✅ Found {len(db_products)} toilets products in database")
    ...
    print("\n2. Connecting to Toilets Google Sheet...")
    sheets_manager = SheetsManager()
    toilets_config = get_collection_config('toilets')

    print(f"   Collection: {toilets_config.name}")
    print(f"   Spreadsheet ID: {toilets_config.spreadsheet_id}")
    print(f"   Worksheet: {toilets_config.worksheet_name}")

    # Get all products from sheet
    print("\n3. Reading products from Google Sheet...")
    all_rows = sheets_manager.get_all_products('toilets')
