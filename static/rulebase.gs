...
function processRowSafely(row, allRules, dimensionMap) {
  ...
  try {
    ...
    // Cubic Weight (Column AP = index 41) - based on dimensions U, V, W (indices 20, 21, 22)
    if (!row[41] && row[20] && row[21] && row[22]) {
...
function calculateCubicWeight(lengthMm, widthMm, heightMm, addPallet = true, conversionFactor = 250) {
  ...
  try {
    // Parse dimensions and ensure they're numbers
    let length = parseFloat(lengthMm.toString());
    let width = parseFloat(widthMm.toString());
    let height = parseFloat(heightMm.toString());

    if (isNaN(length) || isNaN(width) || isNaN(height)) return null;
    if (length <= 0 || width <= 0 || height <= 0) return null;

    // Add 100mm pallet height if requested
    if (addPallet) {
      height += 100;
    }
    ...
    return Math.round(cubicWeight * 10) / 10;

  } catch (error) {
    console.log('Error calculating cubic weight:', error);
    return null;
  }
}
...
function testCubicWeightCalculation() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      const addPallet = testCase.addPallet !== false; // Default to true unless explicitly false
