#!/usr/bin/env python3
"""
SMART TOILETS AI EXTRACTION SCRIPT

This script extracts ALL smart toilet fields from PDF spec sheets
and updates the Google Sheet using AI extraction.

Extracts:
- Standard toilet specs (installation_type, trap_type, actuation_type, etc.)
- Dimensions (pan_height, pan_width, pan_depth, overall dimensions)
- Power & Electrical (power_rating_watts, voltage, frequency_hz, etc.)
- Smart Features (bidet_wash, heated_seat, warm_air_dryer, etc.)
- Temperature Controls (water_temp_adjustable, seat_temp_adjustable)
- Hygiene Features (self_cleaning_nozzle, uv_sterilization)
- Controls (control_type)

Usage:
    python3 extract_smart_toilets.py
"""
...
COLLECTION_NAME = 'smart_toilets'
BATCH_SIZE = 5  # Write to sheets every N products (smaller batch for smart toilets)
...
def main():
    print("=" * 80)
    print("SMART TOILETS AI EXTRACTION")
    print("=" * 80)
    print("\nThis script extracts ALL smart toilet specifications from PDF spec sheets")
    ...
    print("📥 Step 2: Fetching all Smart Toilets products...")
    ...
    for row_num, product in products_dict.items():
        ...
        # Check if key smart toilet fields are missing (need extraction)
        has_power_rating = product.get('power_rating_watts', '').strip()
        has_bidet_wash = product.get('has_bidet_wash', '').strip()
        has_heated_seat = product.get('has_heated_seat', '').strip()
        has_control_type = product.get('control_type', '').strip()

        # Only process if has PDF spec sheet AND missing smart toilet fields
        if spec_sheet and '.pdf' in spec_sheet.lower():
            # Process if missing ANY of the key smart toilet fields
            if not all([has_power_rating, has_bidet_wash, has_heated_seat, has_control_type]):
    ...
    for i, product in enumerate(products_to_process, 1):
        ...
        try:
            ...
            # Rate limit - slightly longer for smart toilets due to complex extraction
            time.sleep(2)
    ...
    print()
    print("✅ All Smart Toilets data has been written to Google Sheets!")


def write_batch_to_sheets(sheets: SheetsManager, updates: list):
    """Write a batch of smart toilet data updates to Google Sheets"""
