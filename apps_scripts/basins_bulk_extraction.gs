/**
 * BULK AI EXTRACTION FOR BASINS - Google Apps Script
 *
 * This script enables bulk AI extraction for the Basins collection.
 * Extracts product specifications from PDF spec sheets and populates the Google Sheet.
 *
 * SETUP INSTRUCTIONS:
 * 1. Open your Basins Raw_Data Google Sheet
...
const COLLECTION_NAME = 'basins';

// Column numbers in BASINS sheet (1-indexed) - matches BasinsCollection column_mapping
const BASINS_COLUMNS = {
  URL: 1,                    // Column A - URL
  ...
  TITLE: 6,                  // Column F
  VENDOR: 7,                 // Column G
  COLOUR: 8,                 // Column H - AI extracts this
  ...
  LOCATION: 20,              // Column T - AI extracts this
  DRAIN_POSITION: 21,        // Column U - AI extracts this
  SPEC_SHEET: 35,            // Column AI - shopify_spec_sheet (PDF URLs - USED for extraction)
};
...
function countProductsReady() {
  ...
  const specSheetColumn = sheet.getRange(2, BASINS_COLUMNS.SPEC_SHEET, lastRow - 1, 1).getValues();
  const installationTypeColumn = sheet.getRange(2, BASINS_COLUMNS.INSTALLATION_TYPE, lastRow - 1, 1).getValues();
...
function bulkExtractRows(rowNumbers) {
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast(
    'Starting bulk extraction for ' + rowNumbers.length + ' basins...',
    '🔄 Extracting',
    5
  );
...
Column Mappings:
  Spec Sheet: Column AI (${BASINS_COLUMNS.SPEC_SHEET})
  Installation Type: Column I (${BASINS_COLUMNS.INSTALLATION_TYPE})
  Colour: Column H (${BASINS_COLUMNS.COLOUR})
  Material: Column J (${BASINS_COLUMNS.PRODUCT_MATERIAL})
  Style: Column L (${BASINS_COLUMNS.STYLE})
  Location: Column T (${BASINS_COLUMNS.LOCATION})
  Drain Position: Column U (${BASINS_COLUMNS.DRAIN_POSITION})
